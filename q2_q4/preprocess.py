...
def preprocess_word_representations_to_numbers(text):
    ...
    for i, word in enumerate(converted_words):
        if word == "-" and i > 0 and i < len(converted_words) - 1 and converted_words[i - 1].isdigit() and converted_words[i + 1].isdigit():
...
if __name__ == "__main__":
    ...
    # Compare for Q2
    df1 = pd.read_csv("project_data_cleaned.csv", usecols=["rowID", "Q2: How many ingredients would you expect this food item to contain?"])
